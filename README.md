...
**Berrycake.js** は、設定ファイルと JavaScript で動かす名前変換スクリプトです。  
デフォルト名を書いた設定ファイルと JavaScript をアップロードし、各ページで読み込むだけで運用できます。

> **注意**  
> 本リポジトリはあくまで **バックアップ / アーカイブ** 用として公開しているため、  
...
- InternetExplorer 等の一部環境では動作しません。  
- ファイルのアップロード・タグ打ちなど、ある程度の Web サイト編集知識が必要です。  
- 変換対応タグは初期設定で `p, li, ruby, rt, h1, h2, h3, h4, h5, a, th, td, span, div` のみです。  
...
5. **HTML で `berrycake.js` を読み込み、変換範囲を設定**  
   - `<script src="berrycake.js"></script>` を `<body>` の閉じタグ直前に書く形がおすすめです。  
